...
from ..models.lsdm import UNet2DConditionLSDModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = LSDPipeline.from_pretrained("hyper-human", torch_dtype=torch.float16)
        >>> pipe = pipe.to("cuda")
...
class LSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    # adapted from https://github.com/huggingface/diffusers/blob/v0.21.4/src/diffusers/pipelines/controlnet/pipeline_controlnet.py#L589
    ...
    # apapted from https://github.com/huggingface/diffusers/blob/v0.21.4/src/diffusers/pipelines/stable_diffusion_xl/pipeline_stable_diffusion_xl.py#L506
    ...
    def __call__(
        ...
        Args:
            ...
            image (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`):
                The LSD input condition to provide guidance to the `unet` for generation. If the type is
                specified as `torch.FloatTensor`, it is passed to LSD as is. `PIL.Image.Image` can also be
                accepted as an image. The dimensions of the output image defaults to `image`'s dimensions. If height
                and/or width are passed, `image` is resized accordingly. This condition is concatenated channel-wise to
                to noise latents    
        ...
        num_branches = self.unet.config.num_branches
        latents = torch.stack(
            [
        ...
        conditioning_image = self.prepare_image(
            image=image,
            width=width,
            height=height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * 2, dim=1) if do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                # cat latents and conditions along channel dim
                latent_model_input = torch.cat([latent_model_input, conditioning_image], dim=2)
                ...
                if do_classifier_free_guidance:
                    noise_pred_texts = []
                    noise_preds_cfg = torch.empty_like(latents)
                    for i, noise_pred in enumerate(noise_preds):
                        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                        noise_preds_cfg[i] = noise_pred
                        noise_pred_texts.append(noise_pred_text)
                    noise_preds = noise_preds_cfg

                if do_classifier_free_guidance and guidance_rescale > 0.0:
                    # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                    noise_preds_cfg_rescale = torch.empty_like(latents)
                    for i, (noise_pred, noise_pred_text) in enumerate(zip(noise_preds, noise_pred_texts)):
                        noise_pred = rescale_noise_cfg(
                            noise_pred, noise_pred_text, guidance_rescale=guidance_rescale
                        )
                        noise_preds_cfg_rescale[i] = noise_pred
                    noise_preds = noise_preds_cfg_rescale
                
                # compute the previous noisy sample x_t -> x_t-1
                latents = torch.stack(
                    [
                        self.scheduler.step(
                            noise_pred, t, latents_i, **extra_step_kwargs, return_dict=False
                        )[0] for noise_pred, latents_i in zip(noise_preds, latents)
                    ]
                )
